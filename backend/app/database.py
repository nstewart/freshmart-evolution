...
async def collect_container_stats():
    """Background task to collect CPU and memory stats every 5 seconds for both PostgreSQL and Materialize"""
    logger.info("Starting container stats collection task")
    
    # Initialize stats for both containers
                    ...
                    if len(stats_str) == 2:
                        ...
                        try:
                            ...
                            async with stats_lock:
                                ...
                                logger.debug(f"Updated {stats_key} stats: CPU={cpu_usage}%, MEM={mem_usage}%, measurements={len(stats['timestamps'])}")
                                
                        except ValueError as e:
                            logger.error(f"Error converting stats strings for {container_name}: {str(e)}")
                    else:
...
async def get_container_stats():
    """Get CPU and memory usage stats for both PostgreSQL and Materialize"""
    current_time = time.time()
    response = {
        "timestamp": int(current_time * 1000)
    }
